...
foreach my $line (@lines) {
	...
	while($line =~ m/\B(?=(\%\w|&\s*\%\w|[+-]\s*\(\s*.*?\s*\)))/gc) {
		...
		if($line =~ /\G%hook\s+([\$_\w]+)/gc) {
			...
			my $replacement = "static __attribute__((constructor)) void _logosLocalCtor_".substr(md5_hex($`.$lineno.$'), 0, 8)."(int __attribute__((unused)) argc, char __attribute__((unused)) **argv, char __attribute__((unused)) **envp)";
			...
			my $replacement = "static __attribute__((destructor)) void _logosLocalDtor_".substr(md5_hex($`.$lineno.$'), 0, 8)."(int __attribute__((unused)) argc, char __attribute__((unused)) **argv, char __attribute__((unused)) **envp)";
			patchHere($replacement);
